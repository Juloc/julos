...
Refresh landed for `local` and `https` in the same work item. A refresh opens one content-locked view of the source, reads `catalog.json`, and reads and digest-checks every file the index declares before a single row is written; only a source that parsed completely replaces the cached entries, in one transaction. A failed refresh therefore leaves the previous catalog in place, marks it `stale` and records the stable failure code, and a source that has never succeeded reports `never` rather than a guessed state. The refresh runs under a durable Operation rather than inside the request, and requesting the same refresh twice while one is queued or running joins that operation instead of racing a second one against the same cache.

The source digest is the immutable identity the adapter locked to. `git` and `oci` will supply one natively; `local` and `https` have none, so the refresh locks to the digest of the index, which binds every entry by its own digest in turn. That difference is visible in one place: an unchanged native identity covers the whole source and lets a refresh stop early, while an unchanged index digest does not, so every file is still read and checked before the refresh concludes that nothing changed.
...
The `git` adapter reads `https://host/owner/repo.git`, optionally followed by `#branch-or-tag`. It materializes one commit with a shallow, single-branch, tag-free, non-recursive clone and reports that commit as the source digest, which is what makes a refresh from a moving branch reproducible. `git` ships in the Server image and is invoked as a fixed argument vector with no shell between (decision `D046`); everything an administrator configured is positional after `--`, so a location or reference beginning with a hyphen is a value rather than an option. Only HTTPS remotes are accepted, because the other transports Git speaks authenticate with a private key and would mean writing that key to disk for the duration of a refresh, and the HTTPS credential travels in the child process environment rather than on the command line. Git runs with no terminal prompt and a five-minute deadline, so an unauthenticated or unresponsive remote is a failed refresh rather than a process that waits.

All five source kinds are implemented.
...
`DefinitionDigest` is SHA-256 over RFC 8785 JSON Canonicalization Scheme bytes of the app manifest. Decision `D044` narrows that to what two implementations can agree on: numbers are integers a double represents exactly, anything else fails with `catalog.definition_invalid`, and a leading byte-order mark is skipped before parsing so the canonical form never carries one. The entry digest in the index covers the published bytes including any mark; the definition digest covers the canonical form. Every executable or displayed referenced file has its own digest in that manifest, so the definition digest binds the complete declared bundle. The exact signature input is UTF-8 `julos-app-definition-v1\n<lowercase-definition-sha256>\n`; the first accepted algorithm is ECDSA P-256/SHA-256 P1363 over those bytes. `artifactSha256` must equal `DefinitionDigest`.
