...
Schema initialization and upgrade run only through the explicit `JulOS.Server --migrate-database` command or the equivalent one-shot Compose service; normal Server startup never changes the schema and manual schema edits are unsupported. PostgreSQL and SQLite both apply committed, ordered migrations. PostgreSQL uses Entity Framework migrations recorded in `__ef_migrations_history`; SQLite uses the Infrastructure-owned ordered script runner recorded in `__julos_schema_history`, where every applied script is checksummed and immutable. `EnsureCreated` is permitted only for isolated test stores that are never upgraded. An existing SQLite database is adopted only when its complete schema fingerprint matches the recorded baseline; an unknown, partially upgraded or newer schema fails with `database.sqlite_schema_unsupported` and is never guessed at, repaired or downgraded (decision `D043`).

PostgreSQL and SQLite migrations enforce equivalent identities, valid revisions, scope shape, layout mode/nullability and lifecycle rules with provider-appropriate keys, partial indexes, foreign keys and check constraints. PostgreSQL uses a `BEFORE UPDATE OR DELETE` append-only audit trigger; SQLite uses the equivalent pair of `RAISE(ABORT)` triggers, because it supports neither a combined trigger event nor a procedural language. Because SQLite cannot add a constraint to an existing table, the parity migration rebuilds the affected tables with the generalized `ALTER TABLE` procedure from the SQLite manual; rows that violate a rule this release enforces make the migration fail and roll back rather than being deleted or rewritten. Domain/Application rules remain the first validation layer, but a supported provider may not weaken a documented persistence invariant merely because its DDL syntax differs.
