...
| BRW-004 | Full Browser application | In progress | The Browser app has an address field, start/stop, live session status and a Guacamole display. A profile selector now lists the user's retained profiles through the `interactive.profiles` capability and starts the session with the chosen profile (temporary by default); its pure logic is covered by `packages/JulOS.Browser/frontend/test` under the new `browser-frontend-test` validation stage. The toolbar also offers a new-profile form that creates a persistent profile against an existing network profile through `interactive.profiles/create`, and a tab bar runs multiple independent sessions per window (switching detaches and re-attaches the display, background sessions persist under the server idle timeout, and closing a tab terminates its session). Application-mode profile creation and downloads (via FILE-007) remain; Back/Forward is handled inside the remote Chromium chrome rather than by JulOS. |
| BRW-005 | Fixed web-application mode | In progress | Launching the Browser app from a saved application target now renders minimal app chrome (address bar, tabs, profile selector and save/new controls hidden) around the same full interactive browser session — not an iframe — with a "Full browser" control that reveals the complete chrome. `resolveChromeMode`/`appLaunchTitle` are covered by `packages/JulOS.Browser/frontend/test`. Durable application-profile (`ApplicationKey`) linkage and a persisted per-target chrome-mode policy remain. |
