...
| REM-008 | SSH provider integration | In progress | Issue #45 adds explicit password/private-key/NONE authentication, host-key verification, timeout, keepalive and terminal policy on the existing encoder; repository and deployed validation remain. |
| Phase 6 | Remote and Browser | In progress | REM-001 through REM-004 are complete. REM-005 through REM-008 have repository-side implementations or active implementation branches; deployed validation and Browser remain. |
...
REM-008 uses the same flat encoder and one additive options record. It validates password, OpenSSH private-key and NONE authentication; strict single-entry host-key verification; bounded timeout and keepalive; and explicit terminal font settings. Caller-owned private-key and passphrase memory remains provider-local, omitted `SshOptions` preserve the previous SSH payload, and SSH options supplied to RDP or VNC fail closed. Transport version `0.3.0` is reserved for this additive SSH policy.
