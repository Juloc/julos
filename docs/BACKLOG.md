...
| Phase 6 | Remote and Browser | In progress | REM-001 through REM-004 are complete; REM-005 functional display transport and client is next. Existing package shells are not complete display implementations. |
...
The functional same-origin display data path, resizing, input, full-screen client and transport-specific presentation behavior belong to REM-005. REM-004 does not add a placeholder display endpoint.
