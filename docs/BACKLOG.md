...
| REM-003 | Shared transport implementation | Done | `JulOS.Remote.Transport` 0.1.0 is the initial tested, immutable and attested implementation consumed by JulOS Remote and Julgate; later provider-policy versions remain additive. |
...
| REM-007 | VNC provider integration | In progress | Issue #43 adds additive explicit VNC authentication, resize, clipboard, display and bounded retry policy on the existing encoder; repository validation and deployed VNC evidence remain. |
| Phase 6 | Remote and Browser | In progress | REM-001 through REM-004 are complete. REM-005 through REM-007 have repository-side implementations with deployed validation still open. SSH and Browser remain planned. |
...
REM-007 keeps VNC ownership in the same provider boundary and adds no new orchestration layer. The existing JSON-auth encoder validates and maps password authentication, dynamic or fixed display resize, clipboard direction and encoding, cursor behavior, color depth, read-only and local-input policy, compression, quality and a bounded retry count. Omitting `VncOptions` preserves the previous payload, while VNC options supplied to RDP or SSH fail closed. Transport version `0.2.0` is reserved for the additive RDP and VNC provider-policy release so immutable publication does not overwrite `0.1.0`.
