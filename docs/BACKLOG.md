...
| BRW-001 | Isolated Chromium runtime image | In progress | Issue #47 adds a pinned unprivileged Chromium image, internal VNC endpoint, bounded Runtime Manager definition, health/cleanup logic and immutable attested GHCR publication; repository and publication validation remain. |
| Phase 6 | Remote and Browser | In progress | REM-001 through REM-004 are complete. REM-005 through REM-008 have repository acceptance with deployed evidence open. BRW-001 is implemented on its validation branch; BRW-002 through BRW-005 remain. |
