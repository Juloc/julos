...
| BRW-002 | Browser proxy compatibility | In progress | Transparent host proxy compatibility includes framing/CSP handling, cookie/redirect behavior, WebSockets, HTTPS preservation behind Caddy and cross-origin redirect re-encoding. Dynamic/public targets no longer receive JulOS forwarding metadata, avoiding canonical redirect loops against encoded proxy hosts. Media/origin/body compatibility and real-site acceptance remain. |
