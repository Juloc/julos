...
| BRW-002 | Browser profiles and network profiles | Done | User-bound persistent/application profiles, non-persistent temporary mode, exact administrator-allowlisted Runtime Manager networks, opaque proxy secret references and provider-neutral SQLite/PostgreSQL package metadata storage are implemented. |
...
| Phase 6 | Remote and Browser | In progress | Remote repository acceptance is largely complete with deployed evidence open. BRW-001 and BRW-002 are implemented; BRW-003 through BRW-005 remain. |
