...
| BRW-001 | Unified proxy Browser | In progress | One user-facing Browser. The existing Core proxy address-bar becomes the primary Browser surface; legacy Browser/Adaptive Browser package applications are hidden from the launcher. Proxy mode remains default. |
| BRW-002 | Browser proxy compatibility | In progress | Continue transparent host proxy compatibility: framing/CSP handling, cookie/redirect/origin behavior, WebSockets, media and managed application targets. Incompatibility is reported inside the same Browser. |
