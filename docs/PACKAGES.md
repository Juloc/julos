...
`PKG-014` made the signature optional and the states reachable. An upload may carry no signature at all, in which case it is `unsigned`; it may carry a signature together with a public key this installation has no configuration for, in which case a valid signature makes it `unknown-signed`. A supplied key can never produce `trusted-signed` — trust comes from configuration, so an artifact describing its own key proves only that whoever wrote the artifact also wrote the key — and an upload claiming an identity this installation *does* have a key for while supplying different key material is refused outright with `package.publisher.key_conflict` rather than downgraded to unknown. A claimed signature that cannot be completed at all is `invalid` and fails the install: half a claim is still a claim of authenticity. The digest is settled before any of this, and a mismatch always fails, because a claim about who produced bytes means nothing until it is settled which bytes those are. `invalid-signature` is therefore never a stored state — it is a refusal.

Installing anything that is not silently safe goes through a preview first. `POST /api/v1/packages/previews` takes the identical upload, changes nothing, and returns the package identity and version, the signature state and publisher, the declared permissions and runtime rights, the sorted warning codes, whether the package would run isolated, and an acknowledgement digest. `POST /api/v1/packages/install` then repeats the upload with that digest. Official packages use `POST /api/v1/packages/catalog/{packageId}/previews` and pass the digest in the install body; signing does not exempt them, because signing a package does not make its rights safe.

The acknowledgement is a digest over the whole assessment together with the operation key, not a stored token. The server recomputes it from what was actually uploaded rather than trusting what the caller says it approved, so an approval cannot be replayed for different bytes, a different version, different rights, a different publisher or a different operation — each of those produces a different digest. Nothing is persisted between the two calls, so there is no window in which an approval exists for an artifact the server no longer has. A missing or foreign acknowledgement fails with `409 package.acknowledgement_required`.

Warnings are stable codes: `package.warning.unsigned`, `package.warning.unknown_publisher`, `package.warning.isolated_runtime` and `package.warning.critical_rights`. The last one is raised for a trusted publisher too whenever the package declares permissions, a worker image or network access. A package that asks for nothing and is trusted needs no acknowledgement at all, because a confirmation nobody needs is what teaches an administrator to click through the ones that matter.
