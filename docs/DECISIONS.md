...
Local accounts and roles use ASP.NET Core Identity stores in the existing `core` schema. Browser sessions use one persistent HTTP-only, same-site-strict Identity cookie scoped to `/`; the default lifetime is 48 hours with sliding expiration, and authenticated status refreshes the cookie during Desktop boot so reopening JulOS renews the full lifetime. Desktop/mobile presentation and user-agent changes on the same origin do not create separate authentication state. The antiforgery cookie shares the same same-site/security policy. Both mark themselves secure exactly when the inbound request is HTTPS (`CookieSecurePolicy.SameAsRequest`) rather than unconditionally, because the documented single-container alpha deployment binds Kestrel to plain loopback HTTP by design and ASP.NET Core's antiforgery middleware raises a hard `InvalidOperationException` on every request when `SecurePolicy` is `Always` and the request is not HTTPS. Server installs an authenticated-user fallback policy, and only setup, login, authentication status and health probes may opt out explicitly.
