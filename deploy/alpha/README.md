...
- access to `ghcr.io/juloc/julos-server:0.3.0-alpha.11`
...
mkdir -p julos-alpha
cd julos-alpha
curl -O https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.11/deploy/alpha/compose.sqlite.yaml
curl -o .env.example https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.11/deploy/alpha/.env.sqlite.example
cp .env.example .env
...
JulOS answers on `http://127.0.0.1:8080` unless the bind address or port was changed. The server image contains the published Desktop web root, so `/` serves the JulOS shell directly. On a fresh database, open JulOS and create the initial administrator. Setup is accepted only once.
...
```bash
curl -O https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.11/deploy/alpha/compose.yaml
curl -o .env.example https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.11/deploy/alpha/.env.example
cp .env.example .env
```
...
No Core schema change exists between alpha.10 and alpha.11. Existing alpha.10 SQLite volumes upgrade in place; no volume recreation or database edit is required.
