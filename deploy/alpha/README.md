...
- access to `ghcr.io/juloc/julos-server:0.3.0-alpha.3`
...
mkdir -p julos-alpha
cd julos-alpha
curl -O https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.3/deploy/alpha/compose.sqlite.yaml
curl -o .env.example https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.3/deploy/alpha/.env.sqlite.example
cp .env.example .env
...
SQLite is intended for one server replica on one host. Do not share its database file between containers or hosts. Process package workers use their own isolated SQLite files and are supported in this profile. Container package runtimes still require the Runtime Manager transport and its deployment-specific Docker access.
...
```bash
curl -O https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.3/deploy/alpha/compose.yaml
curl -o .env.example https://raw.githubusercontent.com/Juloc/julos/v0.3.0-alpha.3/deploy/alpha/.env.example
cp .env.example .env
```
