...
- additive explicit Apache Guacamole 1.6.0 SSH authentication, host-key, network and terminal policy
...
The published 0.1.0 `GuacamoleLaunchRequest` constructor remains intact. Existing consumers that omit `RdpOptions`, `VncOptions` and `SshOptions` keep their prior behavior. New JulOS provider integrations use the additive explicit options and are validated before any launch payload is created.
...
Source, security and migration rules are documented in `docs/REMOTE-TRANSPORT-PACKAGE.md`, `docs/RDP-PROVIDER-CONTRACT.md`, `docs/VNC-PROVIDER-CONTRACT.md` and `docs/SSH-PROVIDER-CONTRACT.md` in the JulOS repository.
